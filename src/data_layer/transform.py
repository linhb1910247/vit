...
class LoadHW(object):
    ...
    def __call__(self, data):
        ...
        idx_img = data['idx_img']
        print(idx_img)  # In giá trị của idx_img
        print(self.tsv)  # In toàn bộ self.tsv để xem cấu trúc của nó
        print(self.tsv[idx_img])  # In giá trị trả về từ self.tsv[idx_img]
        key, str_hw = self.tsv[idx_img][0], self.tsv[idx_img][1]    # Đoạn mã mới
        print("key:",key)
        print(str_hw)
        if 'key' not in data:
            data['key'] = key
        print("idx_img:", data['idx_img'])
